    ...
    with open(data_path, 'r') as f:
        for line_num, line in enumerate(f):
            ...
            for step in game_trajectory:
                ...
                # Skip if observation is not a dict
                ...
                if game == 'uno':
                    ...
                    sft_item = _convert_gin_rummy_step(observation, action, history)
...
def _convert_gin_rummy_step(obs, action, history):
    ...
    try:
        ...
        item = prompt_gin_rummy % (
            ...
            obs.get('player_id', 0),
            json.dumps(obs.get('hand', [])),
            json.dumps(obs.get('top_discard', None)),
            json.dumps(obs.get('dead_cards', [])),
            json.dumps(obs.get('opponent_known_cards', [])),
            json.dumps(obs.get('stock_pile_num', 0)),
            json.dumps(history),
            json.dumps(legal_actions),
        )
        return {
