...
from util.douzero_util.llm_agent import LLMAgent
...
pretrained_dir = 'pretrained/douzero_WP'
players = []
for position in ['landlord', 'landlord_down', 'landlord_up']:
    players.append(LLMAgent(llm_function, prompt_function_dou_dizhu, out_parse_function_dou_dizhu))
...
def predict():
    if request.method == 'POST':
        try:
            ...
            # Number cards left dict
            num_cards_left_dict = {
                'landlord': num_cards_left[0],
                'landlord_up': num_cards_left[2],
                'landlord_down': num_cards_left[1],
            }
            ...
            info_set.num_cards_left_dict = num_cards_left_dict
            ...
            # Prediction
            actions = players[player_position].act(info_set)
            # actions = [''.join([EnvCard2RealCard[a] for a in action]) for action in actions]
            actions = [''.join([EnvCard2RealCard[a] for a in actions])]
            actions_confidence = [1]
            result = {}
            win_rates = {}
